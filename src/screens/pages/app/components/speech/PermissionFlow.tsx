...
export const PermissionFlow = ({
  ...
  const [permissionTargetName, setPermissionTargetName] = useState("this app");
  const [restartInstruction, setRestartInstruction] =
    useState("Then quit and restart the app.");

  useEffect(() => {
    const loadPermissionCopy = async () => {
      setPermissionTargetName(await getPermissionTargetName());
      setRestartInstruction(await getPermissionRestartInstruction());
    };
  ...
  const stateConfig = {
    ...
    requesting: {
      ...
      description: `Enable ${permissionTargetName} in System Settings (${checkAttempts}/20)`,
            ...
            <div className="mt-3 space-y-2">
              ...
              {showManual && (
                <ol className="text-xs text-muted-foreground space-y-1 list-decimal list-inside pt-2 border-t border-border/50">
                  ...
                  <li>Enable {permissionTargetName}</li>
                  <li>{restartInstruction}</li>
                </ol>
              )}
            </div>
          )}
