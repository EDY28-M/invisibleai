...
// Patrones estáticos de clasificación
const FILLER_EXACT_PHRASES: &[&str] = &[
    "ok", "okay", "ya", "vale", "bueno", "este", "eh", "em", "um", "gracias",
    "perfecto", "listo", "continuamos", "vamos a continuar", "siguiente"
];

const SMART_QUESTION_PATTERNS: &[&str] = &[
    "que opinas", "como lo harias", "como lo resolverias", "que propones",
    "cual seria", "por que", "puedes explicar", "puedes aclarar",
    "que estrategia", "que alternativa", "que solucion", "que enfoque",
    "que harian", "que recomiendas", "que sugieres", "what do you think",
    "how would you", "what would you suggest", "can you explain", "why would"
];

const SMART_REQUEST_PATTERNS: &[&str] = &[
    "necesitamos", "ayudanos", "ayudame", "explica", "resuelve", "resolver",
    "recomienda", "recomendar", "sugiere", "sugerencia", "mejorar", "mejorarias",
    "how can we", "help us", "recommend", "suggest"
];

const SMART_OPINION_PATTERNS: &[&str] = &[
    "yo creo que", "creo que", "considero que", "me parece que",
    "desde mi punto de vista", "en mi opinion", "para mi", "pienso que",
    "i think", "i believe", "in my opinion", "from my point of view"
];

const SMART_OBJECTION_PATTERNS: &[&str] = &[
    "no estoy de acuerdo", "discrepo", "no necesariamente", "no lo veo viable",
    "no me convence", "eso no tiene sentido", "eso no escala", "eso esta mal",
    "eso es falso", "el problema con eso", "el problema es", "hay un problema",
    "i disagree", "that does not scale", "that is wrong", "that is not viable",
    "the problem is"
];

const SMART_WEAK_CONTRAST_PATTERNS: &[&str] = &[
    "pero", "sin embargo", "aunque", "however", "but"
];

const SMART_DEBATE_CLAIM_PATTERNS: &[&str] = &[
    "siempre", "nunca", "es mejor", "es peor", "deberia", "no deberia",
    "lo correcto seria", "la mejor forma", "no sirve", "no funciona",
    "es obligatorio", "es necesario", "always", "never", "is better",
    "is worse", "should", "should not", "does not work"
];

const SMART_RISK_PATTERNS: &[&str] = &[
    "riesgo", "riesgoso", "caro", "costoso", "demasiado tiempo", "seguridad",
    "compliance", "cumplimiento", "deuda tecnica", "rendimiento", "performance",
    "latencia", "mantenible", "mantenimiento", "escala", "escalabilidad",
    "risk", "expensive", "security", "technical debt", "latency", "scalability"
];

const SMART_PROPOSAL_PATTERNS: &[&str] = &[
    "propongo", "mi propuesta", "propuesta", "deberiamos", "podriamos",
    "prioricemos", "el plan seria", "la alternativa es", "vamos a elegir",
    "we should", "we could", "my proposal", "the proposal is", "the plan is"
];

const SMART_DECISION_PATTERNS: &[&str] = &[
    "decision", "decidir", "hay que decidir", "la decision seria",
    "vamos a decidir", "elegir", "prioridad", "priorizar", "we need to decide",
    "the decision", "priority", "prioritize"
];

const SMART_LIVE_CLASS_PATTERNS: &[&str] = &[
    "en el chat preguntan", "alguien pregunta", "no entendi", "puedes explicar",
    "que significa", "eso es falso", "eso esta mal", "como se hace",
    "sirve para que", "someone asks", "in chat they ask", "what does it mean",
    "i did not understand"
];

/// Normaliza un texto para buscar coincidencias (minúsculas y sin acentos ni puntuación común)
fn normalize_text(text: &str) -> String {
    let mut normalized = text.to_lowercase();
    // Reemplazar acentos comunes para búsquedas más amplias
    normalized = normalized.replace('á', "a")
                           ...
                           .replace('ü', "u")
                           .replace('ñ', "n");
    // Filtrar puntuación básica
    normalized.retain(|c| !c.is_ascii_punctuation());
    normalized
}

/// Clasifica un texto del sistema audio de forma inteligente
pub fn classify_smart_system_event(text: &str) -> EventClassification {
    let normalized = normalize_text(text);
    
    // 1. Filtrar ruido/frases de relleno
    ...
    let check_pattern = |patterns: &[&str], kind: StreamingEventKind, score: u32, reason: &str, candidates: &mut Vec<Candidate>| {
    ...
    check_pattern(SMART_QUESTION_PATTERNS, StreamingEventKind::ExternalQuestion, 7, "question_pattern", &mut candidates);
    check_pattern(SMART_REQUEST_PATTERNS, StreamingEventKind::ExternalRequest, 6, "request_pattern", &mut candidates);
    check_pattern(SMART_OBJECTION_PATTERNS, StreamingEventKind::ExternalObjection, 7, "objection_pattern", &mut candidates);
    check_pattern(SMART_WEAK_CONTRAST_PATTERNS, StreamingEventKind::ExternalObjection, 4, "weak_contrast_pattern", &mut candidates);
    check_pattern(SMART_PROPOSAL_PATTERNS, StreamingEventKind::ExternalProposal, 6, "proposal_pattern", &mut candidates);
    check_pattern(SMART_DECISION_PATTERNS, StreamingEventKind::ExternalDecision, 6, "decision_pattern", &mut candidates);
    check_pattern(SMART_LIVE_CLASS_PATTERNS, StreamingEventKind::ExternalRequest, 6, "live_or_class_pattern", &mut candidates);
    check_pattern(SMART_RISK_PATTERNS, StreamingEventKind::ExternalObjection, 5, "risk_pattern", &mut candidates);
    ...
    if opinion_matched {
        candidates.push(Candidate {
            kind: if debate_matched { StreamingEventKind::ExternalDebateClaim } else { StreamingEventKind::ExternalOpinion },
            score: if debate_matched { 6 } else { 4 },
            reason: if debate_matched { "opinion_with_debate_claim".to_string() } else { "opinion_pattern".to_string() },
        });
    ...
    // Orden de prioridad por tipo de evento
    ...
    EventClassification {
        event_kind: if trigger_score >= 5 { best.kind } else { StreamingEventKind::ExternalContext },
        trigger_score,
        confidence,
        reasons,
    }
...
/// Clasificación para el modo estándar (solo preguntas y peticiones directas)
pub fn classify_standard_system_event(text: &str) -> EventClassification {
    ...
    EventClassification {
        event_kind: if is_actionable { classification.event_kind } else { StreamingEventKind::ExternalContext },
        trigger_score: if is_actionable { classification.trigger_score } else { 0 },
        confidence: classification.confidence,
        reasons: classification.reasons,
    }
}

/// Clasifica si una frase del micrófono es un comando explícito al asistente
pub fn detect_mic_command(text: &str) -> bool {
    let normalized = normalize_text(text);
    let trigger_words = &["invisible", "asistente", "sugiere", "oye", "corrige", "ayuda", "dime"];
    
    // Si contiene alguna de las palabras clave al inicio o de forma explícita
    trigger_words.iter().any(|&word| normalized.contains(word))
}

/// Cancela la tarea del LLM activa en este momento
...
/// Ingesta y orquestación principal para segmentos de audio recibidos
pub async fn handle_incoming_transcript(
    ...
    // 1. Guardar el segmento en la base de datos
    ...
    // Notificar al frontend que el timeline cambió
    let _ = app.emit("timeline-updated", &session_id);

    // 2. Determinar si se dispara la IA
    let should_trigger = if is_mic {
        // En micrófono, solo disparar si es un comando explícito
        detect_mic_command(&text)
    } else {
        // En sistema audio, clasificar según el modo
    ...
    // 3. Cancelar llamada LLM previa (si existe)
    cancel_active_llm_task(&app).await;

    // 4. Iniciar ejecución LLM en segundo plano y guardarlo en el AppState
    ...
    let task_handle = tokio::spawn(async move {
        if !is_mic {
            // Si viene del sistema audio, debouncer de 350ms para esperar fin de frase o interrupción
        ...
        match run_llm_orchestration(&app_clone, &session_id_clone, &text_clone, is_mic).await {
            Ok(full_response) => {
                // Insertar respuesta del asistente en SQLite
...
/// Ejecuta la consulta LLM, maneja la carga de credenciales y timeline, y realiza el stream
async fn run_llm_orchestration(
    ...
    // Reconstruir la línea de tiempo cronológica de SQLite
    let timeline_segments = session_service::get_combined_session_timeline(
        app_data_dir.clone(),
        session_id,
        15, // últimas 15 frases
    )?;

    // Cargar prompt contextual unificado
    ...
    // Mapear historial en formato OpenAI de forma cronológica
    ...
    // Agregar la última frase como disparadora
    ...
    if let Some(ref groq_key) = local_groq_key {
        ...
        request_builder = request_builder.header("Authorization", format!("Bearer {}", api_access_key));
    }
